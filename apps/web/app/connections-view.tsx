...
export function ConnectionsView() {
  ...
  return <section className="resource-view connections-workbench">
    ...
    {deleteTarget && <div className="confirm-backdrop" role="presentation"><div className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-provider-title"><div className="panel-heading"><div><p className="eyebrow"><Trash2 size={14} /> 删除连接</p><h2 id="delete-provider-title">删除 Provider？</h2></div><button type="button" className="icon-button" aria-label="关闭删除确认" title="关闭" onClick={() => setDeleteTarget(null)} disabled={busy}><X size={17} /></button></div><div className="confirm-dialog-body"><p>确定删除“{deleteTarget.name}”吗？已绑定的 Judge 版本不能删除其 Provider。</p><div className="editor-actions"><button type="button" className="outline-button" onClick={() => setDeleteTarget(null)} disabled={busy}>取消</button><button type="button" className="danger-button-text" onClick={() => void deleteProvider(deleteTarget)} disabled={busy}>{busy ? "删除中..." : "确认删除"}</button></div></div></div></div>}
